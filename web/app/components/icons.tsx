...
export function CatIcon({ color = "currentColor", size = 24, className }: { color?: string; size?: number; className?: string }) {
  // 16x16 pixel-art cat face — pointy ears, round head, big eyes, nose, whiskers
  const p = size / 16;
  ...
  // eyes (dark cutout — 2x2 each)
  const eyes: [number, number][] = [
    [4,5],[5,5],[4,6],[5,6],
    [10,5],[11,5],[10,6],[11,6],
  ];

  // eye sparkle (bright dot in each eye)
  const sparkle: [number, number][] = [[5,5],[11,5]];

  // nose
  const nose: [number, number][] = [[7,8],[8,8]];

  // inner ear (slightly transparent for depth)
  const innerEar: [number, number][] = [
    [3,1],[3,2],   // left
    [12,1],[12,2], // right
  ];
